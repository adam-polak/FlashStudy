...
public class UserTableAccess
{
    ...
    private bool UserExists(string username)
    {
        ...
        {
            connection.Open();
            List<UserModel> list = (List<UserModel>)connection.Query<UserModel>($"SELECT * FROM {table} WHERE username='{username}';");
            connection.Close();
            return list.Count() != 0;
        }
    }
    ...
    private bool CorrectLogin(string username, string password)
    {
        ...
        {
            connection.Open();
            List<UserModel> list = (List<UserModel>)connection.Query<UserModel>($"SELECT * FROM {table} WHERE username='{username}' AND password='{password}';");
            connection.Close();
            return list.Count() != 0;
        }
    }
